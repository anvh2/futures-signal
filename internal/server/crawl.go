...
func (s *Server) fetchExchange() error {
...
func (s *Server) fetchMarketSummary() error {
	var (
		wg    = &sync.WaitGroup{}
		total = int32(0)
		start = time.Now()
	...
	for _, interval := range viper.GetStringSlice("market.intervals") {
		...
		go func(interval string) {
			...
			for _, symbol := range s.exchangeCache.Symbols() {
				...
				resp, err := s.binance.ListCandlesticks(ctx, symbol, interval, viper.GetInt("chart.candles.limit"), 0, 0)
				if err != nil {
